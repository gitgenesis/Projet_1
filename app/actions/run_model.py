...
def MC_run(dataframe,num_simulations, num_of_years):
    ...
    inflation_df = dataframe_change['CPI']*100
    ...
    MC_realrate = MCSimulation(
        ...
        num_trading_days = round(12*num_of_years,0)
    )
    ...
    ci_lower_rate = round(real_rate_summary[8]*real_rate_df.iloc[-1],2)
    ci_upper_rate = round(real_rate_summary[9]*real_rate_df.iloc[-1],2)
    ...
    #95% Confidence Interval
    ci_lower_earnings = round(real_rate_summary[8]*earnings_df.iloc[-1],2)
    ci_upper_earnings = round(real_rate_summary[9]*earnings_df.iloc[-1],2)
    ...
    ci_lower_price = PERatio[0] * ci_lower_earnings[0]
    ci_upper_price = PERatio[0] * ci_upper_earnings[0]
